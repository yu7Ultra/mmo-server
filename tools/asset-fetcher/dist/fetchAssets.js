...
function logVerbose(msg) { if (opts.verbose || opts.debug)
    console.log(chalk.gray('[verbose] ' + msg)); }
function logDebug(msg) { if (opts.debug)
    console.log(chalk.magenta('[debug] ' + msg)); }
async function ensureDir(dir) {
    await fs.promises.mkdir(dir, { recursive: true });
}
async function download(entry, outDir, userAgent, dry) {
...
function shouldInclude(name, filters) {
...
async function handleZip(res, cfg) {
    if (res.dryRun)
        return; // skip processing in dry-run
    ...
    try {
        ...
        for (const e of entries) {
            ...
            if (!shouldInclude(entryNameNormalized, res.entry.pathFilters))
                continue;
...
async function runPostAction(action, dir) {
...
async function performSharpExtraction(action, dir) {
    const { frameWidth, frameHeight, pattern, margin = 0, spacing = 0, mode = 'meta', directionOrder, directionBlockSize, metaFile } = action;
    ...
    for (const file of files) {
        ...
        if (mode === 'files')
            await ensureDir(outputFramesDir);
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                const x = margin + c * (frameWidth + spacing);
                const y = margin + r * (frameHeight + spacing);
                if (x + frameWidth > meta.width || y + frameHeight > meta.height)
                    continue;
        ...
        if (directionOrder && directionBlockSize) {
            mapping = {};
            directionOrder.forEach((dirName, dIdx) => {
...
async function processAsset(res, cfg) {
    if (res.entry.type === 'zip')
        await handleZip(res, cfg);
}
async function main() {
...
main();
async function updateAttributions(cfg) {
    const rootAttrPath = path.resolve(__dirname, '../../../ATTRIBUTIONS.md');
    let existing = '';
    try {
        existing = await fs.promises.readFile(rootAttrPath, 'utf-8');
    }
    catch { }
